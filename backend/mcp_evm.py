...
# Use dynamic paths to ensure compatibility across different OS - replaces the hardcoded absolute path
    async with MCPTools(f'python "{SERVER_PATH}"') as mcp_tools:
        agent = Agent(
            model=Ollama(id=os.getenv("LLM_MODEL", "llama3.2")),
            tools=[mcp_tools],
            instructions=dedent("""\
...
if __name__ == "__main__":
    ...
    asyncio.run(run_agent("What is the balance of address 0x70997970c51812dc3a010c7d01b50e0d17dc79c8?"))
